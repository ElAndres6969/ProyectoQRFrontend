...
async function loadCompanies() {
  ...
  error.value = '';
  try {
    const response = await fetch(`http://localhost:4000/api/companies?search=${encodeURIComponent(search.value)}&page=${page.value}`, {
...
async function deleteCompany(idempresa) {
  ...
  try {
    const response = await fetch(`http://localhost:4000/api/companies/${idempresa}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${props.token}` },
    });
    const data = await response.json();
