...
async function loadToken() {
  error.value = '';
  try {
    const res = await fetch('http://localhost:4000/api/attendance/qrcode/me', {
...
async function generateToken() {
  ...
  message.value = '';
  try {
    const res = await fetch('http://localhost:4000/api/attendance/qrcode/generate', {
      method: 'POST',
      headers: { Authorization: `Bearer ${props.token}` },
    });
    const data = await res.json();
...
async function scan() {
  ...
  try {
    const res = await fetch('http://localhost:4000/api/attendance/scan', {
      method: 'POST',
      headers: {
